...
#define DUKGLUE_SIMPLE_VALUE_TYPE(TYPE, DUK_IS_FUNC, DUK_GET_FUNC, DUK_PUSH_FUNC, PUSH_VALUE) \
		...
		template<> 
		struct DukType<DukBuffer> { 
			typedef std::true_type IsValueType; 
			
			template<typename FullT> 
			static DukBuffer read(duk_context* ctx, duk_idx_t arg_idx) { 
				if (duk_is_buffer_data(ctx, arg_idx)) { 				
					duk_size_t out_size = 0;
					void * ptr = duk_get_buffer_data(ctx, arg_idx, &out_size); 
					return DukBuffer{ptr,out_size};
				} else { 
					duk_int_t type_idx = duk_get_type(ctx, arg_idx); 
					duk_error(ctx, DUK_RET_TYPE_ERROR, "Argument %d: expected buffer, got %s", arg_idx, detail::get_type_name(type_idx)); 
				} 
			} 
			
			template<typename FullT> 
			static void push(duk_context* ctx, DukBuffer value) { 
				duk_push_external_buffer(ctx);
				duk_config_buffer(ctx, -1, value.ptr, value.size);
